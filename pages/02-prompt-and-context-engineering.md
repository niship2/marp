...
- **品質の標準化**: 組織全体での品質レベルの統一
- **効率化**: 手動でのプロンプト調整時間の削減
- **継続的改善**: データの蓄積による継続的な最適化
...
## 2-20. コンテキストエンジニアリング周辺の知識(2) **Agent**
...
## 2-26. ハーネスエンジニアリング（Harness Engineering）

#### 2026 年に登場した「第 3 の AI エンジニアリング」

2026 年 2 月、Mitchell Hashimoto が **「Agent = Model + Harness」** という定式を提唱し、
プロンプトエンジニアリング・コンテキストエンジニアリングに続く新しい言葉として
**ハーネスエンジニアリング**が広がり始めました。

| レイヤ                       | 問いかけ                         | 主な対象                        |
| ---------------------------- | -------------------------------- | ------------------------------- |
| プロンプトエンジニアリング   | **何を**尋ねるか                 | 命令文・例示                    |
| コンテキストエンジニアリング | **何を渡す**か                   | RAG、メモリ、ツール戻り値       |
| ハーネスエンジニアリング     | システム**全体をどう動かす**か   | ツール定義、権限、評価、観測    |

> モデルは"馬"。プロンプトは「右に曲がれ」という命令。コンテキストは地図と道路標識。
> ハーネスは **手綱・鞍・柵・道そのもの**――10 頭の馬が同時に安全に走るための設計。
> （Martin Fowler "Harness engineering for coding agent users" より要約）[63]

---

## 2-26. ハーネスエンジニアリング（1）なぜ今これが必要なのか

#### モデル単体の限界

- フロンティアモデルの性能差は縮小し、**モデルは交換可能なコモディティ**化
- 同じモデルでも、**周辺の設計次第**で生産性が 10 倍変わる
- 「プロンプトを工夫する」「コンテキストを盛る」だけでは、運用時の**ドリフト・再発するミス・監査要件**に対応できない

#### Hashimoto の核心的アイデア

> エージェントが間違えるたびに、その間違いを**構造的に再発不能**にする仕組みを環境側に作り込む。

- 個別プロンプトで都度直すのではなく、**ハーネス側に永続的な修正**を埋め込む
- 結果として、レビュー負荷が減り、システム品質が単調増加する

---

## 2-26. ハーネスエンジニアリング（2）構成要素

#### 大きく 2 種類の制御

```
■ Guides（フィードフォワード制御）― 行動の前に方向付ける
   - システムプロンプト / AGENTS.md / CLAUDE.md
   - ツール定義と説明文
   - 権限ポリシー（どのツールをいつ使ってよいか）
   - 制約ファイル（禁止事項・必須記載事項）
   - 入力前処理パイプライン

■ Sensors（フィードバック制御）― 行動の後に観測・自己修正
   - 評価（Evals）と回帰テスト
   - 出力バリデータ・パーサ
   - ガードレール（毒性・PII・秘匿情報チェック）
   - ドリフト検出
   - ヒューマン・イン・ザ・ループの介入点
```
...
## 2-26. ハーネスエンジニアリング（3）主要 6 コンポーネント

| # | コンポーネント                  | 役割                                                       |
| - | ------------------------------- | ---------------------------------------------------------- |
| 1 | **Tools & Orchestration**        | MCP/関数呼び出し、ステップ計画、エージェント間連携        |
| 2 | **Permissions & Security**       | ツール毎の許可、機密データ分類、リージョン制御             |
| 3 | **State & Persistence**          | セッション横断の状態保持、チェックポイント、再開可能性     |
| 4 | **Memory**                       | 短期/長期メモリ、知識ベースの選択的注入                    |
| 5 | **Observability**                | トレース、メトリクス、コスト・トークン計上、監査ログ       |
| 6 | **Evals**                        | ゴールデンセット、回帰テスト、運用 KPI への直結           |

---

## 2-26. ハーネスエンジニアリング（4）3 つのエンジニアリングの関係

```
┌─────────────────────────────────────────────────────────┐
│                  Harness（ハーネス）                     │
│   Tools / Permissions / Memory / Observability / Evals  │
│                                                         │
│   ┌────────────────────────────────────────┐           │
│   │            Context（コンテキスト）       │           │
│   │   RAG / Tool Results / History          │           │
│   │                                         │           │
│   │   ┌──────────────────────────┐         │           │
│   │   │   Prompt（プロンプト）    │         │           │
│   │   │   System + User Message  │         │           │
...
**外側のレイヤほど、変更コストは高いが効果は永続的**。

---

## 2-26. ハーネスエンジニアリング（5）知財業務への適用 ①

#### 知財 AI に必要なハーネスの 4 つの特徴

1. **守秘義務とのトレードオフが大きい**
   - 出願前情報・依頼者情報・先願秘匿は最優先
   - データ越境（EU・中国・米国）の管理が必須
2. **手続的厳密性が要求される**
   - クレーム形式、サポート要件、書式、期限管理
   - 一度の誤りが権利化失敗に直結
3. **長期にわたる文脈の維持**
   - 1 件の特許は出願 → OA → 査定 → 維持 → 訴訟と数十年
   - 案件 (matter) 単位のメモリ設計が要
4. **複数の標準と国法に同時準拠**
   - JPO/USPTO/EPO/CNIPA/KIPO の手続規則差異
   - 弁理士法・利益相反・代理人資格

---

## 2-26. ハーネスエンジニアリング（6）知財業務への適用 ②: Guides 設計

#### Guides（フィードフォワード）の知財向け設計例

```
# Tools（ツール定義）
- search_jplatpat / search_espacenet / search_uspto_pair
- search_internal_docket          ← 自社案件 DB
- check_conflict_of_interest      ← 利益相反チェック必須
- draft_claim / draft_specification ← レビュー必須に固定
- file_to_jpo / pay_annuity       ← 人間承認必須（権限分離）

# Permissions（権限）
- search_*: 自動実行可
- draft_*: 自動実行可、ただし出力は必ず弁理士レビュー
- file_*, pay_*: 二段階承認必須
- 出願前情報を扱うツール: 外部 API 呼び出し禁止

# AGENTS.md / CLAUDE.md（プロジェクト規約）
- 事務所の用語統一表（"comprising" 訳語、"wherein" 訳語 等）
- 必須記載事項（最先の優先日、発明者表記、ファミリー）
- 禁止表現（profanity、断定的な特許性判断）
- 国別ドラフトルール（米国: means-plus-function 注意 等）
```

---

## 2-26. ハーネスエンジニアリング（7）知財業務への適用 ③: Sensors 設計

#### Sensors（フィードバック）の知財向け設計例

```
# 出力バリデータ
- クレーム形式チェック（独立/従属、引用関係、文末"こと"統一）
- アンテシーデント・ベイシスチェック（"the X" の初出確認）
- 明細書サポート要件チェック（クレーム用語の明細書出現）
- 引用文献の実在性チェック（ハルシネーション防止）
- 期限・優先日の算定検証

# ガードレール
- 出願前情報の外部送信検出
- 依頼者間の利益相反検出
- 機密区分（社外秘/出願前/公開後）の自動分類

# Evals（評価）
- ゴールデンセット：シニア弁理士のドラフトとの差分
- 翻訳評価：定訳DBとの一致率
- OA 応答評価：採用率・拒絶克服率
- 先行技術調査評価：適合率/再現率
```
...
## 2-26. ハーネスエンジニアリング（8）知財業務への適用 ④: その他のコンポーネント

#### State / Memory / Observability / Governance

```
■ State & Persistence（状態保持）
   - matter ID 単位のセッション分離（混線防止）
   - OA・補正・査定の履歴を永続化
   - 中断と再開（弁理士レビュー後の継続）

■ Memory（メモリ）
   - 案件メモリ：発明者・技術分野・関連ファミリー
   - 発明者メモリ：好まれる表現・過去の出願傾向
   - 事務所メモリ：ハウススタイル

■ Observability（観測）
   - 案件単位のトークン消費・コスト計上
   - すべての AI 出力に対する監査証跡（EU AI Act 対応）
   - 機密漏洩・利益相反のアラート
...
## 2-26. ハーネスエンジニアリング（9）導入の進め方

#### 段階的構築アプローチ

```
Stage 1: 観測から始める（1〜2 ヶ月）
   - 既存のチャット利用ログを集める
   - 「再発するミス」を分類する
   - 上位 5 件のミスを「構造的に再発不能」にする目標を立てる

Stage 2: Guides を整備（2〜3 ヶ月）
   - AGENTS.md/CLAUDE.md にハウススタイルを文書化
   - ツール定義に権限ゲートを導入
   - 機密分類ルールを明示

Stage 3: Sensors を組む（3〜6 ヶ月）
   - 出力バリデータ（クレーム形式・引用実在性）
   - ゴールデンセットでの回帰テスト
   - ダッシュボード（成功率・コスト・レビュー指摘数）

Stage 4: 継続改善ループ（恒常）
   - 新しいミスが出るたびに、ハーネスへ恒久対策を追加
   - モデルを差し替えてもハーネスは資産として残る
```

---

## 2-26. ハーネスエンジニアリング（10）知財事務所 / 知財部のチェックリスト

```
□ 出願前情報を扱うツールの外部送信を完全に遮断しているか
□ ツール毎に「自動実行可 / レビュー必須 / 二段階承認」を分離しているか
□ AGENTS.md / CLAUDE.md にハウススタイルを文書化しているか
□ 利益相反チェックが AI 起動前のフックに組み込まれているか
□ クレーム・明細書出力に対する形式バリデータが動いているか
□ 引用文献の実在性（ハルシネーション）を検証する仕組みがあるか
□ 案件 ID 単位でセッションとメモリが分離されているか
□ すべての AI 出力に対する監査ログが保存されているか
□ ゴールデンセットによる回帰テストを定期実行しているか
□ モデル切替時にハーネスが資産として再利用できる構造になっているか
```

> **ポイント**: ハーネスは「モデルが変わっても残る競争優位」。
> 事務所・知財部の暗黙知を**ハーネスに書き起こす**作業こそが、AI 時代の知財 DX の本丸。

参考: Martin Fowler [63]、Mitchell Hashimoto "Harness Engineering" [64]、
OpenAI "Harness engineering: leveraging Codex" [65]、
LangChain "The Anatomy of an Agent Harness" [66]、
awesome-harness-engineering [67]、AddyOsmani "Agent Harness Engineering" [68]
...
プロンプト・コンテキスト・ハーネスを設計しても、「**実際に良くなったのか**」を
測れなければ改善できません。Evals（評価）は AI システムの **テスト駆動開発**に
相当し、ハーネスの Sensors 層の中核を成します。

#### Evals が答える 3 つの問い

```
1. Regression: 変更後に過去できていたことが壊れていないか？
2. Comparison: モデル A と B、プロンプト v1 と v2 のどちらが良いか？
3. Production health: 本番運用で品質は維持されているか？
```

---

## 2-27. Evals 駆動開発（1）4 種類の評価手法

| 種別                 | 仕組み                                       | 向く対象                       |
| -------------------- | -------------------------------------------- | ------------------------------ |
| **決定論的評価**     | 正規表現・スキーマ・スコア計算で直接判定     | 形式・数値・必須項目の有無     |
| **コード実行評価**   | 単体テスト、サンドボックス実行で結果検証     | 生成コード、構造化データ生成   |
| **LLM-as-judge**     | 評価用 LLM がルーブリックに基づき採点        | 文章品質、論理性、整合性       |
| **人間評価**         | 専門家がサンプリング採点                     | ゴールデンセット、最終 QA     |

**重ね使いが原則**: 決定論的 → コード実行 → LLM-as-judge → 人間 の順で **コストの低い層から弾く**。

---

## 2-27. Evals 駆動開発（2）知財業務での評価セット例

#### タスク別ゴールデンセット設計

```
■ 先行技術調査
   - precision@10 / recall@10（ハンドラベル付与した正解集合）
   - 引用文献の実在性 100%（決定論的）
   - キーワード網羅性（クエリ展開の質）

■ クレーム生成・補正
   - 形式チェック合格率（独立/従属、引用関係、文末）
   - サポート要件合格率（明細書中の用語存在）
   - シニア弁理士ドラフトとの BLEU/編集距離

■ 拒絶理由応答
   - 採用可能性（弁理士レビュー後の採用率）
   - 引用文献分析の正確性（LLM-as-judge）
   - 補正案の §112 / 36 条 4 項適合（決定論的）

■ 翻訳
   - 用語一貫性 100%（決定論的）
   - クレーム ↔ 明細書整合 100%
   - 流暢性 LLM-as-judge ≧ 4/5

■ 明細書要約
   - 必須記載要素のカバー率（請求項、効果、実施例）
   - 文字数遵守
   - 事実性（ハルシネーション検出）
```
...
## 2-27. Evals 駆動開発（3）LLM-as-judge の落とし穴

#### よくある誤用

```
× 単一プロンプトで「品質を 1〜5 で採点して」だけ指示
   → スコアの分布が歪む、判定理由が一貫しない

× 評価用 LLM と被評価 LLM が同一モデル
   → 自己一致バイアス（self-preference bias）
...
- **ルーブリックを明示**: 各点数の合格条件を箇条書きで定義
- **Chain-of-Thought 採点**: 採点理由を先に書かせ、最後にスコア
- **評価モデルは別系統**: Claude を採点するなら GPT を judge に
- **ペアワイズ + 絶対値**の併用
- **人間評価との一致率（Cohen's κ）**を継続モニタ

---

## 2-27. Evals 駆動開発（4）運用ツールと CI 統合

#### 主要ツール（2026 年時点）

| ツール                          | 強み                                       |
| ------------------------------- | ------------------------------------------ |
| **LangSmith**                   | LangChain 連携、トレース + Evals 統合      |
| **Langfuse**                    | OSS、self-host 可、コスト追跡              |
| **Arize Phoenix**               | OSS、観測 + Evals、OpenTelemetry           |
| **Braintrust**                  | データセット管理 + 比較ダッシュボード      |
| **OpenAI Evals / Azure AI**     | クラウドベンダー純正                       |
| **DeepEval**                    | pytest 互換、CI/CD 親和性高                |

#### CI/CD への組み込み

```
1. PR ごとに小規模ゴールデンセットで回帰テスト
2. メインマージ前にフルセット実行（夜間バッチ）
3. しきい値割れで自動ブロック
4. 本番には A/B テスト + シャドートラフィックで段階展開
5. 本番ログから新しい failure case を継続的にゴールデンセットへ追加
```

---

## 2-27. Evals 駆動開発（5）知財向け実装フローの例

```
Step 1: ゴールデンセット作成（最重要・最初のボトルネック）
   - シニア弁理士に 50〜100 件の代表ケースを選定してもらう
   - 入力（発明開示書 / OA 通知 / 先行技術）と
     理想出力（クレームドラフト / 応答案 / 調査レポート）のペア
   - 評価困難な質的項目はルーブリック化

Step 2: ベースライン測定
   - 現状のプロンプト・モデルでスコアを記録
   - 手動工数（人間がゼロから書く時間）も同時計測

Step 3: 改善ループ
   - プロンプト/コンテキスト/ハーネスを変更
   - スコア変動を計測、退化していないか確認
   - 退化が見つかれば差分を分析、ハーネスへ恒久対策

Step 4: 本番化と継続改善
   - 本番ログから failure をサンプリング
   - 月次でゴールデンセット拡張
   - モデル切替時は必ずフル評価
```

> **ポイント**: ゴールデンセットは **事務所/知財部の競争優位**そのもの。
> モデルが変わっても、評価データは資産として残る。

参考: OpenAI Evals [93]、Langfuse [94]、LangSmith Evaluation [95]、
Arize Phoenix [96]、Braintrust [97]、DeepEval [98]
